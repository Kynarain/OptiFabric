...
git checkout mc1.21.11
.\gradlew build --offline
```

产物在 `build\libs\`:

- `OptiFabric-1.0.0+mc1.21.11.jar` ← **上传这个**
- `OptiFabric-1.0.0+mc1.21.11-sources.jar`(可选,一般不用发)
...
cd C:\Users\kynar\IdeaProjects\OptiFabric
git add -A
git commit -m "OptiFabric 1.0.0+mc1.21.11: OptiFine on Fabric for 1.21.11"
git remote add origin https://github.com/<你的用户名>/<仓库名>.git
git push -u origin mc1.21.11     # 推当前分支;想一起带上 1.20.6 那版再 git push origin main
```

发 Release(可选,但推荐,方便别人直接下载):

```powershell
git tag v1.0.0+mc1.21.11
git push origin v1.0.0+mc1.21.11
```
...
2. **上传文件**:把 `OptiFabric-1.0.0+mc1.21.11.jar` 作为 release 上传,版本名填 `1.0.0+mc1.21.11`,并把 1.21.11 标为对应的游戏版本。
3. **项目描述**:`docs/DESCRIPTION.md` 里给了成套文案 —— "简介"栏粘贴**简要描述**(英文在前、中文在后,CF 要求英文排最前),项目正文粘贴**详细描述**(有中文和英文两版,CF 支持 Markdown)。
