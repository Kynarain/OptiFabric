...
26.x 只对应 **26.1.2** 一个 MC 版本,所以整条线一起升版就是全部:`-Mc` 在这里会被拒绝
(`release\version.ps1` 会直接告诉你"这条线只有一个 MC 版本,直接用 `-Kind` / `-Set` 给整条线升版")。
`release\publish.ps1` 里那张 `$modVersions` 表因此是空的,它是留给"某个 MC 版本要单独用一个版本号"的扩展位。

> 一套源码、每个 MC 版本一个 jar、于是要能只给其中一个产物升版 —— 那是另一条线(1.21.x,十个 MC 版本,
> 在自己的分支上)的做法,也是 `-Mc` 存在的原因;理由与它一样:按 §3,**已发布的内容不能改**,
> 而只改了某一个 MC 版本的行为时,把其余内容没变的 jar 重新构建、重新发布只会在文档与产物之间制造差异。
