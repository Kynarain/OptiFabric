...
- 两条**独立**发布线(jar 不能互相替代):
  ...
  - **26.x** —— Minecraft 26.1.2。**26.1 起游戏未混淆**,官方名即运行名,没有 yarn、也没有真正的 intermediary 可重映射,因此走另一套构建与运行期路径,产物 `OptiFabric-Reforged-1.2.1+mc26.1.2.jar`(移植记录见 [`docs/PORT_26.x.md`](docs/PORT_26.x.md))。
...
**26.x 线**(项目 `v26.x/`,版本基数 1.2.1):

| Minecraft | 产出的 jar | OptiFine 构建 | Java | 真机验证 |
|---|---|---|---|---|
| 26.1.2 | `OptiFabric-Reforged-1.2.1+mc26.1.2.jar` | `preview_OptiFine_26.1.2_HD_U_K1_pre2.jar` | **25** | **已实测正常**(启动/进世界/方块物品生物渲染/抗锯齿/光影/多人) |
...
2. **编译**(需要联网下载依赖,或本地已有 Gradle/Loom 缓存):
   ...
   ```
3. **安装**:把**对应版本**的 jar(`v1.21.x/build/libs/OptiFabric-1.1.0+mc<版本>.jar` 或 `v26.x/build/libs/OptiFabric-Reforged-1.2.1+mc26.1.2.jar`)和 OptiFine 的 jar 一起放进 **该 Fabric 版本自己的 `mods` 目录**。**不要**同时放两份 OptiFine(会报 `DUPLICATED`),也不要放错版本的 OptiFabric jar,更不要拿 1.21.x 的 jar 去跑 26.1.2(或反过来)。
